...
RESNET_ID = "1s33bLIVRlQvcWLoF-kE2HPOOREkquJJW"
VGG_ID    = "1dCgXmAgp3h3wClV6_MgPy86Wx-wLh3W8"
EFF_ID    = "15hFh0WDQEaNHE_wtAkWZvAtfFzfCRfP5"

def download_models():
    if not os.path.exists('resnet_4class.pth'):
        with st.spinner("⏳ Downloading models from Drive... (1-2 min, only first time)"):
            gdown.download(f"https://drive.google.com/uc?id={RESNET_ID}", 'resnet_4class.pth', quiet=False)
            gdown.download(f"https://drive.google.com/uc?id={VGG_ID}",    'vgg_4class.pth',    quiet=False)
            gdown.download(f"https://drive.google.com/uc?id={EFF_ID}",    'eff_4class.pth',    quiet=False)
...
print("✅ app.py with Drive IDs ready!")
